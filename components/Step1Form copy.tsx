...
const formSteps = [
  { name: 'role' as const, question: 'To start, what’s your current or most recent job title?', tooltipText: 'Your job title helps us understand your core responsibilities and expertise level.' },
  { name: 'industry' as const, question: 'What industry or organisation was that in?', tooltipText: 'The industry provides context about the environment you work in, from tech to healthcare.' },
  { name: 'tasks' as const, question: "Select all the tasks you performed in this role.", tooltipText: 'Detailing your tasks allows us to match you with careers that involve similar day-to-day activities.' },
  { name: 'skills' as const, question: 'Select your key skills', tooltipText: 'Your skills are the primary building blocks we use to identify your strengths and find matching career paths.' },
  { name: 'interests' as const, question: 'Which career, industry, or field interests you?', tooltipText: "Your interests help us suggest career paths that you'll find engaging and fulfilling, even outside your current field." },
  { name: 'education' as const, question: "Let's add your education. What's your highest qualification?", tooltipText: 'Your educational background helps us identify careers that align with your qualifications and academic achievements.' },
  { name: 'review' as const, question: 'Review your profile.', tooltipText: '' },
];
...
const educationLevels = [
    "GCSE", "A-Level", "Bachelor’s", "Master’s", "Doctorate", "Certificate"
];
...
const Step1Form: React.FC<Step1FormProps> = ({ onSubmit, onStepChange, currentStepIndex, initialData, journeyStepLabels, totalSteps }) => {
  ...
  const fetchStepData = async () => {
    console.log('fetchStepData called');
    const relevantExperiences = experiences.filter(exp => exp.role);
    if (relevantExperiences.length === 0 || mode !== 'form') {
      console.log('fetchStepData returned early');
      return;
    }
    ...
    if (currentStepIndex === 3 && generatedSkills.length === 0) {
      ...
      console.log('Skills generated:', skillsData);
    }
    if (currentStepIndex === 4 && generatedInterests.length === 0) {
      ...
      console.log('Interests generated:', interestsData);
    }
  };
  ...
  const isFormValid = () => {
    ...
    if (currentStepName === 'skills') return true;
        ...
        return (
          <>
            <div className="flex items-center">
              <label htmlFor={currentStep.name} className="text-3xl font-semibold text-white">
                {currentStep.question}
              </label>
              {currentStep.tooltipText && <Tooltip text={currentStep.tooltipText} />}
            </div>
            <input
              id={currentStep.name}
                ...
                <div className="flex items-center">
                                  <div className="flex items-center">
                                    <label className="text-2xl sm:text-3xl font-semibold text-white">
                                      Which tasks best describe your work as {getArticle(firstExperience.role)} <span className="font-bold text-slate-300">{firstExperience.role}</span>?
                                    </label>
                                    {currentStep.tooltipText && <Tooltip text={currentStep.tooltipText} />}
                                  </div>
                </div>
      ...
      case 3: // Skills
        ...
        return (
            <div className="w-full">
                ...
                <div className="flex items-center">
                  <label className="text-2xl sm:text-3xl font-semibold text-white">{currentStep.question}</label>
                  {currentStep.tooltipText && <Tooltip text={currentStep.tooltipText} />}
                </div>
      ...
      case 4: // Interests
        ...
        return (
            <div className="w-full">
                ...
                <div className="mt-8">
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            onKeyDown={(e) => handleCustomKeyDown(e, customInterestInput, (value) => {
                                const currentInterests = interests ? interests.split(', ').filter(Boolean) : [];
                                if (!currentInterests.includes(value)) {
                                    setInterests([...currentInterests, value].join(', '));
                                }
                            ...
                            onClick={() => {
                                ...
                                if (trimmedValue) {
                                    ...
                                    if (!currentInterests.includes(trimmedValue)) {
                                        setInterests([...currentInterests, trimmedValue].join(', '));
                                    }
                                    setCustomInterestInput('');
                                }
                            }}
                    ...
                    <div className="flex flex-wrap justify-start items-center gap-3 mt-4">
                        {currentInterestsArray.map((interest) => (
                            <span key={interest} className="relative group inline-flex items-center px-4 py-2 text-base font-medium bg-teal-500 text-white rounded-full shadow-md">
                                {interest}
                                <button
                                    type="button"
                ...
                <h3 className="text-xl font-semibold text-slate-400 mt-8 mb-4">Suggestions</h3>
        ...
        return (
            <div className="w-full">
                ...
                <div className="flex items-center">
                  <label className="text-2xl sm:text-3xl font-semibold text-white">{currentStep.question}</label>
                  {currentStep.tooltipText && <Tooltip text={currentStep.tooltipText} />}
                </div>                <div className="mt-8 flex flex-wrap justify-start items-center gap-4 max-w-3xl">
                    ...
                    <div className="relative">
                        <input type="text" placeholder="Other +" value={educationCustomLevel}
                ...
                <div className="mt-12">
                  <label htmlFor="subject" className="text-2xl sm:text-3xl font-semibold text-white">What was your subject area?</label>
